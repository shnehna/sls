    ...
    <section className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[.9fr_1fr]">
      <form onSubmit={handleSubmit} className="studio-card space-y-5 p-6 sm:p-7">
        ...
        <label className="block text-sm font-medium text-slate-200">
          用户名
          <input value={username} onChange={(event) => setUsername(event.target.value.toLowerCase())} autoComplete="username" className="mt-2 w-full rounded-2xl border border-white/10 bg-ink-950/70 px-4 py-3 text-slate-50 outline-none placeholder:text-slate-500 focus:border-ember-300 focus:ring-2 focus:ring-ember-300/20" placeholder="字母、数字、_ 或 -" required />
        </label>
        <label className="block text-sm font-medium text-slate-200">
          昵称
          <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} autoComplete="name" className="mt-2 w-full rounded-2xl border border-white/10 bg-ink-950/70 px-4 py-3 text-slate-50 outline-none placeholder:text-slate-500 focus:border-ember-300 focus:ring-2 focus:ring-ember-300/20" placeholder="希望我们怎么称呼你？" />
        </label>
        <label className="block text-sm font-medium text-slate-200">
          密码
          <input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="new-password" className="mt-2 w-full rounded-2xl border border-white/10 bg-ink-950/70 px-4 py-3 text-slate-50 outline-none placeholder:text-slate-500 focus:border-ember-300 focus:ring-2 focus:ring-ember-300/20" placeholder="至少 8 位，包含字母和数字" required />
        </label>
        <label className="block text-sm font-medium text-slate-200">
          确认密码
          <input value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} type="password" autoComplete="new-password" className="mt-2 w-full rounded-2xl border border-white/10 bg-ink-950/70 px-4 py-3 text-slate-50 outline-none placeholder:text-slate-500 focus:border-ember-300 focus:ring-2 focus:ring-ember-300/20" placeholder="再次输入密码" required />
        </label>
        ...
        <p className="text-center text-sm text-slate-400">已经注册？ <Link to="/auth/login" className="text-amber-100 hover:text-white">去登录</Link></p>
      </form>
