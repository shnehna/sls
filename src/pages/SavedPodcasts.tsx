...
export default function SavedPodcasts() {
  ...
  if (authLoading || loading) return <section className="studio-panel p-8 text-slate-300">Loading saved podcasts…</section>
  if (!user) return <LoginGate title="Log in to view saved podcasts" />

  return (
    <div className="space-y-6 pb-28">
      <header className="studio-panel p-7 sm:p-9">
        <p className="studio-eyebrow">Saved podcasts</p>
        <h1 className="studio-title mt-2 text-5xl">Your personal shelf</h1>
          ...
          {savedPodcasts.map((podcast) => (
            <div key={podcast.id} className="studio-card flex gap-4 p-4">
              ...
              <div className="min-w-0 flex-1">
                <Link to={`/podcast/${podcast.podcastId}`} className="font-semibold text-slate-50 hover:text-ember-100">{podcast.title || `Podcast ${podcast.podcastId}`}</Link>
                {podcast.author && <p className="mt-1 truncate text-sm text-slate-400">{podcast.author}</p>}
                <button type="button" onClick={() => void removeSavedPodcast(podcast.podcastId)} className="mt-4 rounded-full border border-white/10 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:border-danger/40 hover:text-rose-200">Remove</button>
              </div>
            </div>
          ))}
        </div>
...
export function Bookmarks() {
  ...
  if (authLoading || loading) return <section className="studio-panel p-8 text-slate-300">Loading bookmarks…</section>
  if (!user) return <LoginGate title="Log in to view bookmarks" />

  return (
    <div className="space-y-6 pb-28">
      <header className="studio-panel p-7 sm:p-9">
        <p className="studio-eyebrow">Transcript bookmarks</p>
        <h1 className="studio-title mt-2 text-5xl">Saved shadowing lines</h1>
      </header>
      {error && <div className="rounded-2xl border border-danger/30 bg-danger/10 p-4 text-sm text-rose-100">{error}</div>}
      <div className="space-y-3">
        {bookmarks.length === 0 && <div className="rounded-2xl border border-white/10 bg-white/[.05] p-6 text-slate-300">No bookmarks yet.</div>}
        {bookmarks.map((bookmark) => (
          <Link key={bookmark.id} to={`/episode/${bookmark.episodeId}?t=${Math.floor(bookmark.cueStartTime || 0)}`} className="studio-card block p-5">
            <p className="text-base leading-8 text-slate-100">“{bookmark.cueText || bookmark.note || 'Saved line'}”</p>
            <p className="mt-3 font-mono text-[11px] uppercase tracking-[.08em] text-slate-500">{bookmark.episodeTitle || `Episode ${bookmark.episodeId}`}</p>
          </Link>
        ))}
      </div>
    </div>
...
export function Progress() {
  ...
  if (authLoading || loading) return <section className="studio-panel p-8 text-slate-300">Loading progress…</section>
  if (!user) return <LoginGate title="Log in to view progress" />

  return (
    <div className="space-y-6 pb-28">
      <header className="studio-panel p-7 sm:p-9">
        <p className="studio-eyebrow">Practice progress</p>
        <h1 className="studio-title mt-2 text-5xl">Continue listening</h1>
      </header>
      {error && <div className="rounded-2xl border border-danger/30 bg-danger/10 p-4 text-sm text-rose-100">{error}</div>}
      <div className="grid gap-4 md:grid-cols-2">
        {progress.length === 0 && <div className="rounded-2xl border border-white/10 bg-white/[.05] p-6 text-slate-300">No progress yet.</div>}
